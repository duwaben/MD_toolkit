...
double gr[MAXBIN];
double sumgr[MAXBIN];
double cutoff_ReRe, cutoff_ReX, cutoff_XX;
...
void readCutoff()
{	
	const char *name1 = "CUTOFFXX:";
	const char *name2 = "CUTOFFReX:";
	const char *name3 = "CUTOFFReRe:";
	fp = fopen("pairCorrelationFunctions/CUTOFFPEAK_information", "r");

	findWord(name1);
	fscanf(fp, "%lf", &cutoff_XX);
	rewind(fp);
	findWord(name2);
	fscanf(fp, "%lf", &cutoff_ReX);
	rewind(fp);
	findWord(name3);
	fscanf(fp, "%lf", &cutoff_ReRe);
...
int main(int argc, char const *argv[])
{
	...
	double sumpCN;
	int CUTOFFBIN;
	double pCN_XX, pCN_ReX, pCN_XRe, pCN_ReRe;
	//X-X
	step = 0;
	...
	//pCN
	sumpCN = 0;
	CUTOFFBIN = floor(cutoff_XX/delta_r);
	for(int k = 0; k < CUTOFFBIN; ++ k)
	{
		sumpCN += (delta_r * (k+0.5)) * (delta_r * (k+0.5)) * sumgr[k] * delta_r;
	}
	pCN_XX = sumpCN * 4 * Pi * (NATOM-thisNumPerType.getmax(site)+thisNumPerType.getmin(site))/(LATTICE * LATTICE * LATTICE);
	cout << pCN_XX << endl;
	...
	//pCN
	sumpCN = 0;
	CUTOFFBIN = floor(cutoff_ReRe/delta_r);
	for(int k = 0; k < CUTOFFBIN; ++ k)
	{
		sumpCN += (delta_r * (k+0.5)) * (delta_r * (k+0.5)) * sumgr[k] * delta_r;
	}
	pCN_ReRe = sumpCN * 4 * Pi * (thisNumPerType.getmax(site)-thisNumPerType.getmin(site))/(LATTICE * LATTICE * LATTICE);
	...
	//pCN
	sumpCN = 0;
	CUTOFFBIN = floor(cutoff_ReX/delta_r);
	for(int k = 0; k < CUTOFFBIN; ++ k)
	{
		sumpCN += (delta_r * (k+0.5)) * (delta_r * (k+0.5)) * sumgr[k] * delta_r;
	}
	pCN_ReX = sumpCN * 4 * Pi * (NATOM-thisNumPerType.getmax(site)+thisNumPerType.getmin(site))/(LATTICE * LATTICE * LATTICE);
	pCN_XRe = sumpCN * 4 * Pi * (thisNumPerType.getmax(site)-thisNumPerType.getmin(site))/(LATTICE * LATTICE * LATTICE);
	cout << pCN_ReX << endl;
	cout << pCN_XRe << endl;

	double WC_ReX;
	WC_ReX = 1.0 - pCN_ReX/((NATOM-thisNumPerType.getmax(site)+thisNumPerType.getmin(site))/(double)NATOM) /(pCN_ReX + pCN_ReRe);
	fp = fopen("pairCorrelationFunctions/WC_ReX", "W");
	fprintf(fp, "WC_ReX:%lf\n", WC_ReX);
	fclose(fp);
