...
{  
	...
	for(T = 0; T < TCOR; T ++){
		fprintf(fp, "%lf\t%lf\n", T * 0.001 * POTIM, quotient_1[T]);
	}
	fclose(fp);
	
	fp = fopen("dynamicalProperty/VAF_2", "w");
	for(T = 0; T < TCOR; T ++){
		fprintf(fp, "%lf\t%lf\n", T * 0.001 * POTIM, quotient_2[T]);
	}
	fclose(fp);

	fp = fopen("dynamicalProperty/VAF_3", "w");
	for(T = 0; T < TCOR; T ++){
		fprintf(fp, "%lf\t%lf\n", T * 0.001 * POTIM, quotient_3[T]);
	}
	fclose(fp);
